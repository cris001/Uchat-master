...
public class UserProfileEditItemActivity extends UI implements View.OnClickListener {
    ...
    private void initActionbar() {
        ...
        toolbarView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(key==UserConstant.KEY_NICKNAME){
                    if(updateServer("nickname",editText.getText().toString().trim())){
                    ...
                    if(updateServer("birthday",birthText.getText().toString())){
                    ...
                    if(updateServer("gender",String.valueOf(gender))){
                    ...
                    if(updateServer("phone",editText.getText().toString().trim())){
                        update(editText.getText().toString().trim());
                    }
                    else {
                        return;
                    ...
                    if(updateServer("email",editText.getText().toString().trim())){
                        update(editText.getText().toString().trim());
                    }
                    else {
                        return;
                    ...
                    if(updateServer("signature",editText.getText().toString().trim())){
                        update(editText.getText().toString().trim());
                    }
                    else {
                        return;
    ...
    //将更新的数据传递到服务器中
    public boolean updateServer(String str1,String str2){
        final HashMap<String,String> map=new HashMap<>();
        map.put("account", DemoCache.getAccount());
        map.put(str1,str2);
        map.put("type",str1);
        new Thread(){
            @Override
            public void run() {
                try {
                    if(!(new GetUserInfo().SendDataToServer(map))){
                        Toast.makeText(UserProfileEditItemActivity.this,"本地服务器连接失败！",Toast.LENGTH_SHORT).show();
                        a="false";
                    }
                } catch (IOException e) {
                    e.printStackTrace();
