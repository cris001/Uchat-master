...
public class LoginActivity extends UI implements OnKeyListener {
            ...
            public void onSuccess(LoginInfo param) {
                ...
                new Thread(){
                    ...
                    public void run() {
                        ...
                        map.put("u_account",account);
                        try {
                            String limit=new GetUserInfo().GetLimitFromLocalServer(map);
                            Log.i("------------",limit);
                            DemoCache.setPermission(limit);
                        }catch (Exception e){
                            e.printStackTrace();
                        }
    ...
    private void register() {
        ...
        //判定是否将数据传到本地服务器中，如果上传成功才可以继续进行网易云端的注册
        new Thread(){
            @Override
            public void run() {
                HashMap<String,String> map=new HashMap<>();
        ...
        ContactHttpClient.getInstance().register(account, nickName, password, new ContactHttpClient.ContactHttpCallback<Void>() {
            ...
            public void onSuccess(Void aVoid) {
                DemoCache.setPermission("student");
