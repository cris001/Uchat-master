...
//            sb.append("希望").append("你").append("审批请假 ");
        } else if (message.getType() == SystemMessageType.DeclineTeamInvite) {
            sb.append(fromAccount).append("拒绝了群 ").append(teamName).append(" 邀请");
//            sb.append(fromAccount).append("不同意你的请假");
        } else if (message.getType() == SystemMessageType.ApplyJoinTeam) {  //+permission考慮
//            sb.append("申请加入群 ").append(teamName);
            sb.append("希望").append("你").append("审批请假 "+ (TextUtils.isEmpty(message.getContent()) ? "" : "：" + message.getContent()));
        } else if (message.getType() == SystemMessageType.RejectTeamApply) {
//            sb.append(fromAccount).append("拒绝了你加入群 ").append(teamName).append("的申请");
            if (UserPreferences.getHolidayAllow() == 1){
                sb.append(fromAccount).append("  同意你的请假");
                UserPreferences.setHolidayAllow(0);
            }else {
                sb.append(fromAccount).append("  不同意你的请假");
                UserPreferences.setHolidayAllow(1);
            }
//            sb.append(fromAccount).append("不同意你的请假");
